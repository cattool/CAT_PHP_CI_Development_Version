...
function readParameters(fieldNameArray)
{
	...
	for(var i = 0; i< fieldNameArray.length ; i++)
	{
		...
		{
			parameterString += "&" + fieldNameArray[i] + "=" + $('input[name='+fieldNameArray[i]+']:checked').val();
		}
		else 
		{
...
function logout()
{
	...
	window.open("/logout");
	setTimeout('document.location="/logout"',1000);
}
...
function toggleDisplay(id, browser, imgPath)
{
	...
	{
		$div.show(500);
		$("#" + id + "_img").attr("src",imgPath+"img/open_folder_" + browser + ".gif");
	}
	else
	{
		$div.hide(500);
		$("#" + id + "_img").attr("src",imgPath+"img/closed_folder_" + browser + ".gif");
	}
}
