...
<div id="content-and-context" style="overflow:auto;">
  <div class="wrapper" style="overflow:auto;">
    <div id="content" style="overflow:auto;">
      <div id="breadcrumbs">
        <p><a href="http://www.usask.ca/gmcte/">The Gwenna Moss Centre for Teaching Effectiveness</a> &gt; <a href="/cat">Curriculum Alignment Tool</a> &gt; My Courses </p>
      </div>
      <div id="administration" class="module" style="overflow:auto;"> <br/>
...
else
{
	for(CourseOffering offering : sections)
	{
		...
		%>
          <li><a href="/cat/auth/courseOffering/characteristicsStart.jsp?course_offering_id=<%=offering.getId()%>"> <%=offering.getCourse().getSubject()%> <%=offering.getCourse().getCourseNumber()%> section <%=offering.getSectionNumber()%> (<%=offering.getTerm()%>) </a></li>
          <%
	}
}
%>
		*/
		$course = $this->course_model->getTeachingCourseOfferings($this->session->userdata('username'));
		$course_count = $course->num_rows();
		
		if($course_count > 0){
			$course_data = $course->result();
			foreach($course_data as $rsCourse){
		?>		
				<li>
                	<a href="<?php echo site_url(); ?>/courseOffering/characteristicsStart?course_offering_id=<?php echo $rsCourse->course_offering_id;?>"> <?php echo $rsCourse->subject;?> <?php echo $rsCourse->course_number; ?> section <?php echo $rsCourse->section_number ?> (<?php echo $rsCourse->term ?>) </a></li>	
		<?php
        	}
			
		}else{
			echo "You don't appear to have taught any sections recently";	
		}
		?>
        </ul>
      </div>
